...
Looking at the `is_closed` attribute, it is FALSE for all restaurants. It is not informative, so I will remove it.
```{r}
...
Remove them, since there are only 6, and location is important to the analysis
```{r}
restaurants <- restaurants %>% 
  filter(!is.na(latitude) &  !is.na(longitude))
```
